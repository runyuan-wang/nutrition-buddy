...
> 为圆酱量身定制的营养学AI桌面应用
> 版本：v0.2（新增四大支柱：多智能体会诊 / 专家分身 / 记忆系统 / Skill 生态）
> 日期：2026-08-14

---

## 〇、v0.2 四大支柱（LearnBuddy 对标，范式继承长生工作台）

总体方案见 `docs/LEARNBUDDY_PARITY_PLAN.md`。落地模块（主进程 `src/main/`）：

| 支柱 | 模块 | 运行时数据 |
|------|------|-----------|
| 多智能体会诊 | `agent-db.ts` + `consult-engine.ts` + `ConsultWorkbench.tsx`（含顾问复制模板） | `data/agents.json`、`data/consults/*.json` |
| 多模型路由 | `model-router.ts`（五路 adapter，缺 key 回退主模型） | 设置内 providers（key 不回显渲染端） |
| 记忆系统 | `memory-db.ts` + `MemoryView.tsx`（档案 + 跨会话记忆 + 检索注入） | `data/memory/profiles/*.json`、`data/memory/memory.jsonl` |
| Skill 生态 | `skill-db.ts` + `SkillsView.tsx`（两态注册表，逐项启停 + 全生命周期：文件导入/更新覆盖/删除/导出分享包/分享包导入） | `data/skills/registry.jsonl`、`data/skills/custom/*.md` |
| 专家分身 | `avatar-db.ts`（多分身 + 补丁状态机 proposed→reviewed→applied/rejected + `draftAvatarFromMaterials` 素材→AI提炼草稿向导） | `data/avatars/*.md`、`data/avatars/patches.jsonl` |

M5 创作侧对标 LearnBuddy：分身 5 步链路（素材导入 → AI 提炼 → 本人校准 → 启用）与 Skills 共建共享（导出/导入 .json 分享包）已落地；无 Key 环境向导降级为结构化模板，诚实标注。

长生三铁律贯穿：高危只 `review_required` 永不 blocked；模型输出是候选须终审；永不自动改写知识底座。
